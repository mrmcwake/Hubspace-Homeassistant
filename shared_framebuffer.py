...
class SharedFramebufferContext:
    ...
    """
    def __init__(self, resource: Light, bridge: HubspaceBridge):
        ...
        async with self._state_lock:
            try:
                ...
                if not framebuffer:
                    ...
                    # Try to determine bulb count from the bulb_index
                    expected_bulb_count = max(12, bulb_index + 1)  # At least 12 bulbs, or enough for this index
                ...
                # CRITICAL: Update our cached framebuffer to the new state
...
def get_shared_context(resource: Light, bridge: HubspaceBridge) -> SharedFramebufferContext:
    """
    ...
    if resource.id not in _shared_contexts:
        _shared_contexts[resource.id] = SharedFramebufferContext(resource, bridge)
        LOGGER.info(f"Created shared framebuffer context for device {resource.id}")
