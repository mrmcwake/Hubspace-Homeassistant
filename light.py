...
def _add_entity(entities, hs, model, deviceClass, friendlyName, debug):
    ...
    elif model == "HB-10521-HS":
        _LOGGER.debug("creating string lights")
        parentString = HubspaceLight(hs, friendlyName, debug)
        entities.append(parentString)
        ...
        for [
            ...
            if deviceClass == "fan":
                ...
                if (model == "HB-10521-HS"):
                    _LOGGER.debug("creating a bulb")
                    for lightIndex in range(12):

                        bulb = HubspaceStringLightBulb(
                            light,
                            lightIndex,   
                            hs,
                            friendlyName,
...
class HubspaceLight(LightEntity):
    ...
    def update(self) -> None:
        ...
        """
        self._fullState = self._hs.getAllState(self._childId)

        self._state = self._hs.extractStateValue(self._fullState, "power")
        ...
        if ColorMode.ONOFF not in self._supported_color_modes:
            self._brightness = _brightness_to_hass(
                 self._hs.extractStateValue(self._fullState, "brightness")
            )

        if any(mode in COLOR_MODES_COLOR for mode in self._supported_color_modes):
            c = self._hs.extractStateValue(self._fullState, "color-rgb")
            if c is None:
                self._rgbColor = None
            else:
                r = int(c.get("color-rgb").get("r"))
                g = int(c.get("color-rgb").get("g"))
                b = int(c.get("color-rgb").get("b"))
                self._rgbColor = (r, g, b)

        if (
            any(mode in COLOR_MODES_COLOR for mode in self._supported_color_modes)
            or ColorMode.COLOR_TEMP in self._supported_color_modes
        ):
            self._colorMode = self._hs.extractStateValue(self._fullState, "color-mode")
            self._color_temp = self._hs.extractStateValue(self._fullState, "color-temperature")
            if (
...
class HubspaceStringLightBulb(HubspaceLight):
    """Representation of a bulb belonging to a string light."""
    ...
    def __init__(
        ...
        self._parentString = parentString
        self._bulbIndex = bulbIndex
    
    @property
    def name(self) -> str:
        """Return the display name of this light."""
        return self._name + "_bulb_" + str(self._bulbIndex)

    @property
    def unique_id(self) -> str:
        """Return the display name of this light."""
        return self._childId + "_" + str(self._bulbIndex)
    ...
    def is_on(self) -> bool | None:
        """Return true if light is on."""
        if self._parentString._state is None:
            return None
        else:    
            if (self._parentString._state == "on"):
                fullState = self._parentString._fullState
                colorMode = self._hs.extractStateValue(fullState, "color-mode")
                if (colorMode == "individual"):
                    customColor = self._hs.extractStateValue(fullState, "color-individual", "custom")
                    sequence = self._hs.extractStateValue(fullState, "color-sequence-v2", customColor)
                    frame = sequence.get("color-sequence-v2").get("frameBuffer").get("framebuffer")[self._bulbIndex]

                    brightness = _brightness_to_hass(frame.get("colorBrightness"))
    ...
    def set_individual(self) -> None:
         colorMode = self._hs.extractStateValue(self._fullState, "color-mode")
         if (colorMode != "individual"):
            _LOGGER.error("DEBUG settting color mode")
            self._hs.setState(self._childId, "color-mode", "individual")


    def turn_on(self, **kwargs: Any) -> None:
        self._fullState = self._hs.getAllState(self._childId)
        if (not self._parentString.is_on):
            self._parentString.turn_on()
    
        self.set_individual()

       
        customColor = self._hs.extractStateValue(self._fullState, "color-individual", "custom")
        sequence = self._hs.extractStateValue(self._fullState, "color-sequence-v2", customColor)
        frame = sequence.get("color-sequence-v2").get("frameBuffer").get("framebuffer")[self._bulbIndex]

        #if (frame.get("colorBrightness") == 0):

        if (self._brightness == 0):
            frame["colorBrightness"] = 100
        ...
        if ATTR_BRIGHTNESS in kwargs and (
            ...
            brightness = kwargs.get(ATTR_BRIGHTNESS, self._brightness)
            frame["colorBrightness"] = _brightness_to_hubspace(brightness)
            self._brightness = brightness
        ...
        if ATTR_RGB_COLOR in kwargs and any(
            ...
            frame["r"] = kwargs[ATTR_RGB_COLOR][0]
            frame["g"] = kwargs[ATTR_RGB_COLOR][1]
            frame["b"] = kwargs[ATTR_RGB_COLOR][2]
            r = int(frame.get("r"))
            g = int(frame.get("g"))
            b = int(frame.get("b"))
            self._rgbColor = (r, g, b)
            print("SETTING RGB COLOR TO " + str(self._rgbColor))

        _LOGGER.error("turn on debug update" + customColor)
        _LOGGER.error(sequence)
        self._parentString._fullState = self._fullState
        self._hs.setState(self._childId,
            "color-sequence-v2",
            sequence,
            customColor)
    ...
    def turn_off(self, **kwargs: Any) -> None:
        ...
        customColor = self._hs.extractStateValue(self._fullState, "color-individual", "custom")
        sequence = self._hs.extractStateValue(self._fullState, "color-sequence-v2", customColor)
        sequence.get("color-sequence-v2").get("frameBuffer").get("framebuffer")[self._bulbIndex]["colorBrightness"] = 0
    ...
    def update(self) -> None:
        ...
        self._state = self._parentString._state
        self._fullState = self._parentString._fullState

        if self._fullState is None:
            _LOGGER.error("NO FULL STATE")
            return

        colorMode = self._hs.extractStateValue(self._fullState, "color-mode")
        _LOGGER.error("COLORMODE?" + str(colorMode) + "what?")
        if (colorMode == "individual"):
            _LOGGER.error("INDVIUPDATES")
            customColor = self._hs.extractStateValue(self._fullState, "color-individual", "custom")
            sequence = self._hs.extractStateValue(self._fullState, "color-sequence-v2", customColor)
            frame = sequence.get("color-sequence-v2").get("frameBuffer").get("framebuffer")[self._bulbIndex]
            ...
            b = int(frame.get("b"))
            self._rgbColor = (r, g, b)
            self._brightness = _brightness_to_hass(frame.get("colorBrightness"))
            self._colorMode = self._parentString._colorMode
            self._color_temp = self._parentString._color_temp
            self._temperature_suffix = self._parentString._temperature_suffix
            self._color_temp = self._parentString._color_temp
            _LOGGER.error("DEBUG - rgbColor " + str(r) + " " + str(g) + " " + str(b) + " bright " + str(self._brightness))
        else:
            self._supported_color_modes = self._parentString._supported_color_modes
            # ColorMode.ONOFF is the only color mode that doesn't support brightness
            if ColorMode.ONOFF not in self._supported_color_modes:
                self._brightness = self._parentString._brightness

            if any(mode in COLOR_MODES_COLOR for mode in self._supported_color_modes):
                self._rgbColor = self._parentString._rgbColor
