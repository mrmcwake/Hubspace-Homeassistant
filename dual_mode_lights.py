"""
Dual Mode Light Components for Commercial Electric 12in LED Flush Mount
...
def should_create_dual_lights(resource: Light) -> bool:
    ...
    if result:
        ...
        LOGGER.info(f"Device {device_name} has dual-mode capability - creating separate color and white entities")
    return result
...
class HubspaceColorLight(HubspaceLight):
    ...
    def __init__(self, bridge: HubspaceBridge, controller: LightController, resource: Light) -> None:
        ...
        device_name = _get_device_name(resource)
        self._attr_name = f"{device_name} Color"
...
class HubspaceWhiteLight(HubspaceLight):
    ...
    def __init__(self, bridge: HubspaceBridge, controller: LightController, resource: Light) -> None:
        super().__init__(bridge, controller, resource)
        device_name = _get_device_name(resource)
        self._attr_name = f"{device_name} White"
