...
class HubSpace:
    ...
    def getAllState(self, child):
        ...
        r = requests.get(state_url, headers=auth_header)
        r.close()
        return r.json()
    
    def extractStateValue(self, state, desiredStateName, desiredFunctionInstance = None):
        for value in state.get("values"):
            if (
                value.get("functionClass") == desiredStateName and 
                (desiredFunctionInstance is None or desiredFunctionInstance == value.get("functionInstance"))
            ):
